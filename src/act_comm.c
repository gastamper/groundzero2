...
int
add_page_line (struct char_data *ch, const char *from, time_t time,
              const char *message)
{
    int idx;

    if ( GET_NO_PAGES(ch) >= MAX_PAGER_MEMORY )
        return (-1);

    idx = GET_NO_PAGES(ch)++;
    GET_PAGER_MEMORY(ch) =
        realloc(GET_PAGER_MEMORY(ch),
                sizeof(struct page_data) * GET_NO_PAGES(ch));

    GET_PAGE(ch, idx).from = str_dup(from);
    GET_PAGE(ch, idx).time = time;
    GET_PAGE(ch, idx).message = str_dup(message);

    return (0);
}


/* XXX: I think this code is broken.  First, I used 'goto' for no good reason;
 * additionally, I haven't gone to the cleanUp code in some places I think I
 * should have.  Worse, some of the checks are in "else if" blocks connected
 * to the file loading stuff, so won't ever be reached on a player that's
 * loaded. -sat
 *
 * XXX: Consider removing this in favor of a simple "beep" command to alert
 * someone, with just a generic WAIT_STATE to prevent repeating it.  Keep the
 * NOBEEPS stuff, though. -sat
 */
void
do_page (struct char_data *ch, char *argument)
{
    ...
    {
        send_to_char("Mobiles don't have pagers.\r\n", ch);
        return;
    }
    ...
    if ( !*arg )
    {
        ...
        for ( i = 0; i < GET_NO_PAGES(ch); i++ )
            k += sprintf(buf + k, "%s [%s] :: %s\r\n",
                         GET_PAGE(ch, i).from,
                         no_newline(asctime
                                    (localtime(&GET_PAGE(ch, i).time))),
                         GET_PAGE(ch, i).message);

        k += sprintf(buf + k, "%sYou have %d page%s.\r\n"
                     "\r\n"
                     "Usage: page                    : [List pager's memory]\r\n"
                     "Usage: page <who> <message>    : [Page <who> with <message>]\r\n"
                     "Usage: page clear              : [Clear your pager's memory]\r\n"
                     "Usage: page off                : [Turn off your pager]\r\n"
                     "Usage: page on                 : [Turn on your pager]\r\n"
                     "Usage: beeps                   : [Toggle beep noise on/off]\r\n"
                     "\r\n"
                     "You can page people who aren't online; they'll get the message next time they're\r\n"
                     "online.\r\n", (i ? "\r\n" : ""), i,
                     (i == 1 ? "" : "s"));
    ...
    else if ( !*argument )
    {
        if ( !str_cmp(arg, "clear") )
        {
            ...
            if ( !GET_PAGER_MEMORY(ch) )
            {
                send_to_char("There's nothing to clear...\r\n", ch);
                return;
            }

            send_to_char("Pager memory cleared.\r\n", ch);

            for ( i = 0; i < GET_NO_PAGES(ch); i++ )
            {
                free_string(GET_PAGE(ch, i).from);
                free_string(GET_PAGE(ch, i).message);
            }

            GET_NO_PAGES(ch) = 0;
            free(GET_PAGER_MEMORY(ch));
        }
        else if ( !str_cmp(arg, "off") )
        {
            SET_BIT(ch->comm, COMM_NOPAGE);
            send_to_char("Pager turned off.\r\n", ch);
        }
        else if ( !str_cmp(arg, "on") )
        {
            REMOVE_BIT(ch->comm, COMM_NOPAGE);
            send_to_char("Pager turned on.\r\n", ch);
        }
        else
        {
            send_to_char("Oh, yeah, sure... but page with what?\r\n", ch);
        }

        return;
    }
    else if ( !(target = get_char_world(ch, arg)) )
    {
        if ( !(target = load_char_obj(NULL, arg)) )
        {
            send_to_char("Who in the fuck is that?!\r\n", ch);
            return;
        }
        fromFile = TRUE;
    }
    ...
    {
        send_to_char("Your communication privileges are restricted.\r\n", ch);
        return;
    }
    else if ( IS_NPC(target) || !target->pcdata )
    {
        send_to_char("Mobiles don't have pagers.\r\n", ch);
        return;
    }
    else if ( IS_SET(target->comm, COMM_NOPAGE) )
    {
        act("Can't reach $N's pager -- maybe $E turned it off.  Or forgot to pay the bill.", ch, NULL, target, TO_CHAR);
        goto cleanUp;           /* That's right, GOTO. Nyah. */
    }
    else if ( ch->pcdata->pageDelay && !ch->trust )
    {
        send_to_char("You can't page again, yet.\r\n", ch);
        goto cleanUp;
    }

    ch->pcdata->pageDelay = 2;  /* PAGE_DELAY */

    /*
     * If they're online and not in quiet mode, tell target he's just been
     * paged; otherwise, just put it in the pager memory (if possible) and
     * shut the fuck up about it. -sat
     */
    if ( !fromFile && !IS_SET(target->comm, COMM_QUIET) && target->desc )
    {
        if ( !IS_SET(target->comm, COMM_NOBEEP) )
            send_to_char("\a\a\a", target);
        act("$N just beeped you with: $t", target, argument, ch, TO_CHAR);

        if ( target->desc )
            target->desc->fcommand = TRUE;
    }
    else if ( add_page_line(target, ch->names, time(0), argument) == -1 )
    {
        act("$S pager memory is full or something's broken.", ch, NULL,
            target, TO_CHAR);
        goto cleanUp;
    }
